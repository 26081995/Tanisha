...
public class View_creditor extends JInternalFrame {
	...
	public View_creditor() {
		...
		JLabel lblViewExpense = new JLabel("Creditor");
		...
		try {
		    ...
		    String query = "select ID,Name,Mobile,Balance from Creditor where Company='"+company+"'";
		...
		textField.addKeyListener(new KeyAdapter() {
			public void keyReleased(KeyEvent e) {
				try 
				{
					String query = "select ID,Name,Mobile,Balance from Creditor where (Name like '%"+textField.getText()+"%'  or Mobile like '%"+textField.getText()+"%'  or Balance like '%"+textField.getText()+"%'  ) and Company='"+company+"'";
                    java.sql.PreparedStatement pmt = connection.prepareStatement(query);
                    ResultSet rs = pmt.executeQuery();
                    table.setModel(DbUtils.resultSetToTableModel(rs));
                    pmt.close();
			...
			public void actionPerformed(ActionEvent e) {
				try
                {
					...
					if(dialogResult == JOptionPane.YES_OPTION){
	                    ...
	                    String query1 = "select ID,Name,Mobile,Balance from Creditor where Company='"+company+"'";
	        		    java.sql.PreparedStatement pmt1 = connection.prepareStatement(query1);
