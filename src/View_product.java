...
public class View_product extends JInternalFrame {
	...
	public View_product() {
		...
		JLabel lblViewExpense = new JLabel("View Product");
		lblViewExpense.setFont(new Font("Book Antiqua", Font.BOLD, 25));
		lblViewExpense.setBounds(511, 0, 173, 25);
		...
		try {
		    ...
		    String query = "select ID,Name,HSN,Description,Stock from Product where Company='"+company+"'";
		...
		textField.addKeyListener(new KeyAdapter() {
			public void keyReleased(KeyEvent e) {
				try {
                    ...
                    String query = "select ID,Name,HSN,Description,Stock from Product where (Name like '%"+textField.getText()+"%'  or Description like '%"+textField.getText()+"%' or HSN like '%"+textField.getText()+"%' or Stock like '%"+textField.getText()+"%') and Company='"+company+"'";
                    java.sql.PreparedStatement pmt = connection.prepareStatement(query);
                    ResultSet rs = pmt.executeQuery();
                    table.setModel(DbUtils.resultSetToTableModel(rs));
                    pmt.close();
			...
			public void actionPerformed(ActionEvent e) {
				try
                {
					...
					if(dialogResult == JOptionPane.YES_OPTION){
	                    ...
	                    String query1 = "select ID,Name,HSN,Description,Stock from Product where Company='"+company+"'";
	        		    java.sql.PreparedStatement pmt1 = connection.prepareStatement(query1);
